...
begin
	...
	# Readability
	using Unitful: mm
	using LaTeXStrings, PlutoUI, Printf
end;
...
# ╔═╡ 8e37f46b-bd93-4a0c-8388-b9dc0a505c53
# Import recon 
begin
...
begin
	...
	top_dir = "/mnt/storage/rexfung/20250506ball/recon/"; # top directory
	ksp_path = top_dir * "ksp6x.mat"; # k-space file
	smaps_path = top_dir * "smaps.mat"; # sensitivity maps file
	recon_path = top_dir * "recon/img.mat"; # reconsctruced fMRI file
	
	## Experimental parameters
	(Nx, Ny, Nz, Nc, Nt) = (90, 90, 60, 32, 48) # Tensor size
	(fov_x, fov_y, fov_z) = (216mm, 216mm, 144mm) # Field of view
...
@bind t Slider(1:Nt; show_value=true)
...
begin
	...
	smaps = smaps_raw ./ sqrt.(sum(abs2.(smaps_raw), dims=[1,2,3]))
	@assert all(sqrt.(sum(abs2.(smaps), dims=[1,2,3])) .≈ 1) "Sensitivity maps not normalized"
end;
...
begin
	...
	λ_L = 5e-1 # weight for nuclear norm penalty term
...
begin
	@show nn_cost(X0)
	@show nn_cost(patchSVST(X0, λ_L, patch_size, stride_size))
end
...
git-tree-sha1 = "3540b82a4228ab6352b41d713c876ff08c059ee5"
uuid = "599c1a8e-b958-11e9-0d14-b1e6b2ecea07"
version = "0.12.0"

    [deps.LinearMapsAA.extensions]
    LinearMapsAA_LinearOperators = "LinearOperators"

    [deps.LinearMapsAA.weakdeps]
...
[[deps.MIRT]]
deps = ["AVSfldIO", "Distributed", "FFTW", "FileIO", "ImageFiltering", "Interpolations", "LazyGrids", "LinearAlgebra", "LinearMapsAA", "NFFT", "Random", "SharedArrays", "SparseArrays", "SpecialFunctions", "Wavelets"]
git-tree-sha1 = "12d0a47e36652d74aad7c413318035404a5e630b"
uuid = "7035ae7a-3787-11e9-139a-5545ed3dc201"
version = "0.18.2"

[[deps.MIRTjim]]
deps = ["AxisArrays", "ColorTypes", "FFTViews", "MosaicViews", "OffsetArrays", "Plots", "REPL"]
git-tree-sha1 = "59706c16b89163b5677339f952d87becc645587d"
