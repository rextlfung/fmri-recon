# %% Main9992.jl
# 20260303. Main9991 but trying to use GPU on phobos.
using Pkg
Pkg.activate(".")
...
includet("recon_gpu.jl"); using .recon_gpu: img2patches, patch_nucnorm, patchSVST
...
A = block_diag([Aotazo(CuArray(s), CuArray(smaps)) for s in eachslice(Ω, dims=ndims(Ω))]...)
