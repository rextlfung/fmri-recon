...
fn_recon_base = top_dir * "recon/mslr/6x.mat"; # reconsctruced fMRI file
...
for block in 1:length(block_starts)
    ...
    @assert (N[1], N[2], N[3], Nc, Nt_block) == size(ksp0)
    ...
    ksp = reshape(ksp0, :, Nc, Nt_block)
    ksp = [ksp[vec(s), :, it] for (it, s) in enumerate(eachslice(Ω, dims=4))]
    ksp = cat(ksp..., dims=3) # (Nsamples, Nc, Nt_block), no "zeros"
