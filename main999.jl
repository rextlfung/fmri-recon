# %% Main998.jl
# 20251106 ball + tap data. Pre-computed 
cd("/home/rexfung/github/fmri-recon/")
using Pkg
Pkg.activate(".")
...
top_dir = "/mnt/storage/rexfung/20251003tap/recon/"; # top directory
fn_ksp = top_dir * "rand6x.mat"; # k-space file
fn_smaps = top_dir * "smaps_bart.mat"; # sensitivity maps file
fn_recon_base = top_dir * "fmslr/rand6x.mat"; # reconsctruced fMRI file

# %% Experimental parameters
# EPI parameters
N = (90, 90, 60) # Spatial tensor size
Nvc = 10 # Number of virtual coils
...
for block in 1:length(block_starts)
    ...
    # nearest-neighbor interpolation along time (view-sharing)
    # X0 = dropdims(sum(ifftshift(ifft(fftshift(nn_viewshare(ksp0)), 1:3)) .* conj.(smaps), dims=4), dims=4);
    ...
    # precompute regularization parameters λ
    λ = 1e-3;
    λs = []
    meanλs = zeros(Nscales)
    for k in 1:Nscales
        P = img2patches(X, patch_sizes[k], strides[k])
        Np = size(P, 3)
        σ1s = zeros(Np)
        for p in 1:Np
            σ1s[p] = opnorm(P[:,:,p])
    ...
    function nn_cost(X)
        cost = 0
        for k in 1:Nscales
            cost += patch_nucnorm(img2patches(X, patch_sizes[k], strides[k]), λs[k])
        end
        return cost / Nscales
    end
    ...
    function g_prox(X, c)
        for k in 1:Nscales
            X = patchSVST(X, c.*λs[k], patch_sizes[k], strides[k])
        end
        return X
    end
    ...
    # Save to file
    fn_recon = fn_recon_base[1:end-4] * "_$(λ)_block$block.mat"
